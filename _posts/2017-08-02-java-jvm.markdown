...
![jvm heap][2]
    - <div class="smalltext">`-XX:NewSize和-XX:MaxNewSize` 设置年轻代的大小，建议设为整个堆大小的1/3 或者1/4,两个值设为一样大。</div>
...
* 垃圾回收算法：
    - 标记-清除算法 (Mark-Sweep)
        + <div class="smalltext">标记-清除算法将垃圾回收分为两个阶段：标记阶段和清除阶段。一种可行的实现是，在标记阶段首先通过根节点，标记所有从根节点开始的较大对象<sup>1</sup>。因此，未被标记的对象就是未被引用的垃圾对象。然后，在清除阶段，清除所有未被标记的对象<sup>2</sup>。该算法最大的问题是<u>存在大量的空间碎片</u>，因为回收后的空间是不连续的。在对象的堆空间分配过程中，尤其是大对象的内存分配，`不连续的内存空间的工作效率要低于连续的空间。`</div>
    - 复制算法 (Copying)
